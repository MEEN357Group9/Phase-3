...
elseif  sM(1) ~= sC(1) || sC(1) ~= sK(1) || sM(1) ~= sK(1)
    error('The M,C,and K matrices are not the same size');
elseif sX(2) ~= 1 || sV(2) ~= 1 || sA(2) ~= 1
    error('X0, V0, and A0 must be column vectors.');
elseif sX(1) ~= sV(1) || sV(1) ~= sA(1) || sX(1) ~= sA(1)
...
for i = 2:D.N
    ...
    V(i+1,:) = V2;
    A(i+1,:) = A2;
    [~ , D] = get_forcing_function(T(i+1),D);
